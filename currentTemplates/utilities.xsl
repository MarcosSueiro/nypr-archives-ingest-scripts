...
<xsl:stylesheet version="3.0" 
    ...
    <xsl:variable name="validatingKeywordString" 
        select="'http://id.loc.gov/authorities/subjects/'"/>
    <xsl:variable name="validatingNameString" 
        select="'http://id.loc.gov/authorities/names/'"/>
    ...
    <xsl:template name="ASCIIFy" 
        match="*[matches(., '/P{IsBasicLatin}')]" 
        mode="ASCIIFy" >
        ...
        <xsl:variable name="ASCIIFiedText" select="ASCII:ASCIIFier($inputText)"/>
        <!-- Potentially troublesome characters -->
        <xsl:variable name="nonASCIICodesFound"
            select="distinct-values(string-to-codepoints($inputText)[. gt 125 or . lt 32][. != 9][. != 10][. != 13])"/>
        ...
        <ASCIIResult>
            ...
            <xsl:if test="$nonASCIICharactersFound">
                <charactersRemoved>Non-ASCII characters found: <xsl:value-of
                    select="$nonASCIICharactersFound"/>
                </charactersRemoved>
                <codesRemoved>with codes: <xsl:value-of select="$nonASCIICodesFound"/>
                </codesRemoved>
            </xsl:if>
