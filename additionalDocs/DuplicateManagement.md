...
Given two sonically identical files with matching 'theme' (our suggested definition for duplicate), 
**we propose we keep the file with most recently updated metadata in DAVID**; 
and we will copy any DAVID "REMARKS" (notes) in any files set for deletion to the files to keep.

Once the script determines which files can be erased, their filepaths can be submitted for deletion to administrators.

## 4. Intra-folder results
The result of this deduplication process within 13 DAVID folders identified, between June and November of 2024:
1. **10,277 dupicate sets** within each folder, in groups of duplicates ranging from two to 33 files
2. **11,954 excess WAVE files** representing **over 2,590 GB, or 2.53 TB** of excess data. 





#### Appendix: Sample matching-MD5 document
...
(Note: the second file was also edited later, but pretend we do not know that 😉).
